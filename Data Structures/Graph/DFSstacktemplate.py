...
def DFS_visit(G, u):
    stack = deque()
    stack.append(u) #should append new node in the queue. Previously was appendleft
    G.time += 1
    u.d = G.time
    u.color = "grey"
    while len(stack) != 0:
...
def white_node(G, u):
    for node in G.Adj[u.id]:
        if node.color == "white": #can use yield function (some kind of generators)
            return node
    return None
